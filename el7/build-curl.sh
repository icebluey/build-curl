...
_build_libssh2() {
    ...
    echo
    sed -e '/^Libs/s/-R[^ ]*//g' -e '/^Libs/s/ *$//' -i usr/lib64/pkgconfig/*.pc
    install -m 0755 -d usr/lib64/curl/private
    cp -af usr/lib64/*.so* usr/lib64/curl/private/
    sleep 2
    /bin/cp -afr * /
...
_build_curl() {
    ...
    rm -f curl-*.tar*
    cd curl-*
    LDFLAGS='' ; LDFLAGS="${_ORIG_LDFLAGS}"' -Wl,-rpath,/usr/lib64/curl/private' ; export LDFLAGS
    ./configure \
    --build=x86_64-linux-gnu --host=x86_64-linux-gnu \
    --enable-shared --enable-static \
    ...
    sleep 2
    cd /tmp
    openssl dgst -r -sha256 curl-"${_curl_ver}"-1.el7.x86_64.tar.xz | sed 's|\*| |g' > curl-"${_curl_ver}"-1.el7.x86_64.tar.xz.sha256
    rm -fr "${_tmp_dir}"
    rm -fr /tmp/curl
    /sbin/ldconfig
}
