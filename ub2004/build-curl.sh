...
_build_curl() {
    ...
    rm -f curl-*.tar*
    cd curl-*
    LDFLAGS='' ; LDFLAGS="${_ORIG_LDFLAGS}"' -Wl,-rpath,/usr/lib/x86_64-linux-gnu/curl/private' ; export LDFLAGS
    ./configure \
    --build=x86_64-linux-gnu --host=x86_64-linux-gnu \
    --enable-shared --enable-static \
    ...
    sleep 2
    cd /tmp
    openssl dgst -r -sha256 curl-"${_curl_ver}"-1_amd64.tar.xz | sed 's|\*| |g' > curl-"${_curl_ver}"-1_amd64.tar.xz.sha256
    rm -fr "${_tmp_dir}"
    rm -fr /tmp/curl
    /sbin/ldconfig
}
